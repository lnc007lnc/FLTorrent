...
class FLConfig:
    ...
    DATA_TEXT_FIELD: str = "sentence"         # Text field name in HuggingFace dataset (default: 'sentence' for GLUE)
                                              # Use 'text' for AG News, IMDB, etc.
    DATA_SPLIT_TRAIN_FOR_VAL: float = 0.0     # Ratio of train to split for validation (for datasets without validation set)
                                              # e.g., 0.1 = 10% of train becomes validation (for AG News)

    # === LoRA (Low-Rank Adaptation) Settings ===
    # LoRA enables parameter-efficient fine-tuning for large transformer models
    USE_LORA: bool = False                    # Enable LoRA for transformer models
    LORA_R: int = 8                           # LoRA rank (low rank dimension, common values: 4, 8, 16, 32)
    LORA_ALPHA: int = 16                      # LoRA alpha scaling factor (typically 2*r)
    LORA_DROPOUT: float = 0.1                 # LoRA dropout rate for regularization
    LORA_TARGET_MODULES: List[str] = field(default_factory=list)  # Target modules for LoRA (empty = auto-detect based on model)
    ...
    # === BitTorrent Settings ===
    BITTORRENT_ENABLE: bool = True        # Enable BitTorrent P2P model exchange (False for CFL mode)
    ...
    # === Experiment Settings (for supplementary experiments) ===
    # Bandwidth Heterogeneity (Experiment 1)
    BT_BANDWIDTH_HETEROGENEITY: bool = False  # Enable bandwidth simulation
    BT_SLOW_CLIENT_RATIO: float = 0.5         # Fraction of slow clients (0.5 = 50%)
    BT_FAST_BANDWIDTH_MBPS: float = 100.0     # Fast clients bandwidth (Mbps)
    BT_SLOW_BANDWIDTH_MBPS: float = 10.0      # Slow clients bandwidth (Mbps)

    # Churn Injection (Experiment 2)
    BT_CHURN_INJECTION: bool = False          # Enable churn simulation
    BT_CHURN_RATE: float = 0.0                # Failure probability per round (0.1 = 10%)
    BT_CHURN_MODE: str = "silent"             # 'silent' or 'delayed'

    # Protocol-only Scaling (Experiment 3)
    BT_PROTOCOL_ONLY: bool = False            # Enable protocol-only mode (no ML)
    BT_SYNTHETIC_CHUNK_SIZE: int = 1048576    # Synthetic chunk size (1MB)
    BT_SYNTHETIC_NUM_CHUNKS: int = 16         # Number of synthetic chunks

    # === Topology Settings ===
    TOPOLOGY_USE: bool = True           # Enable topology construction
    ...
    # === Gossip-style DFL Settings ===
    GOSSIP_ENABLE: bool = False           # Enable gossip mode (decentralized FL)
    GOSSIP_MODE: str = "neighbor_avg"     # 'pairwise' | 'neighbor_avg' | 'push_sum'
    GOSSIP_ITERATIONS: int = 1            # Gossip iterations per FL round
    GOSSIP_MIXING_WEIGHT: float = 0.5     # Self model weight (neighbor weight = 1 - w)
    BT_NEIGHBOR_ONLY: bool = False        # Only collect chunks from topology neighbors (for gossip)
    ...
    DATA_ROOT: str = "./data/"  # Data root directory (absolute path for batch experiments)
    ...
    DATA_AUGMENTATION: bool = False           # Enable standard data augmentation for training
                                              # True: RandomCrop(32, pad=4) + RandomHorizontalFlip + ToTensor + Normalize
                                              # False: ToTensor + Normalize only
                                              # Recommended: True for CIFAR/CINIC training from scratch
    DATA_AUGMENTATION_CROP_SIZE: int = 32     # RandomCrop size (32 for CIFAR/CINIC, 84 for CelebA)
    DATA_AUGMENTATION_CROP_PADDING: int = 4   # RandomCrop padding (4 is standard)
    ...
    FEDERATE_IGNORE_WEIGHT: bool = False   # If True, use average aggregation instead of weighted by sample_size
...
class RayV2FederatedLearning:
    ...
    def _build_train_transform(self) -> list:
        """
        Build training transform list based on DATA_AUGMENTATION setting.

        If DATA_AUGMENTATION is enabled, includes:
        - RandomCrop(size, padding) - standard CIFAR/CINIC augmentation
        - RandomHorizontalFlip - 50% probability horizontal flip
        - ToTensor - convert to tensor
        - Normalize - normalize with dataset-specific mean/std

        If DATA_AUGMENTATION is disabled, only includes:
        - ToTensor
        - Normalize

        Returns:
            List of transform specifications in FederatedScope format.
        """
        transforms = []

        if CONFIG.DATA_AUGMENTATION:
            # Standard CIFAR/CINIC data augmentation
            transforms.append([
                'RandomCrop',
                {
                    'size': CONFIG.DATA_AUGMENTATION_CROP_SIZE,
                    'padding': CONFIG.DATA_AUGMENTATION_CROP_PADDING
                }
            ])
        ...
        transforms.append([
            'Normalize',
            {
                'mean': CONFIG.DATA_TRANSFORM_MEAN,
                'std': CONFIG.DATA_TRANSFORM_STD
            }
        ])
    ...
    def generate_base_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            'federate': {
                ...
                'ignore_weight': CONFIG.FEDERATE_IGNORE_WEIGHT  # If True, use average aggregation
            },
            ...
            'data': {
                ...
                # Training transform: with data augmentation if enabled
                'transform': self._build_train_transform(),
                # Validation/Test transform: no augmentation (only ToTensor + Normalize)
                'val_transform': [
                    ['ToTensor'],
            ...
            'train': {
                ...
                'optimizer': {
                    ...
                    # Conditionally add momentum only for SGD (Adam/AdamW don't accept this param)
                    **({'momentum': CONFIG.OPTIMIZER_MOMENTUM} if CONFIG.OPTIMIZER == 'SGD' else {})
                },
            ...
            'topology': {
                'use': CONFIG.TOPOLOGY_USE,
                'type': CONFIG.TOPOLOGY_TYPE,
                'timeout': CONFIG.TOPOLOGY_TIMEOUT,
                'connections': CONFIG.TOPOLOGY_CONNECTIONS,
                'verbose': True,
                'gossip': {
                    'enable': CONFIG.GOSSIP_ENABLE,
                    'mode': CONFIG.GOSSIP_MODE,
                    'iterations': CONFIG.GOSSIP_ITERATIONS,
                    'mixing_weight': CONFIG.GOSSIP_MIXING_WEIGHT
                }
            },
            
            'bittorrent': {
                'enable': CONFIG.BITTORRENT_ENABLE,
                ...
                'neighbor_only_collection': CONFIG.BT_NEIGHBOR_ONLY,  # Gossip: only collect from neighbors
                # Experiment settings
                'bandwidth_heterogeneity': CONFIG.BT_BANDWIDTH_HETEROGENEITY,
                'slow_client_ratio': CONFIG.BT_SLOW_CLIENT_RATIO,
                'fast_bandwidth_mbps': CONFIG.BT_FAST_BANDWIDTH_MBPS,
                'slow_bandwidth_mbps': CONFIG.BT_SLOW_BANDWIDTH_MBPS,
                'churn_injection': CONFIG.BT_CHURN_INJECTION,
                'churn_rate': CONFIG.BT_CHURN_RATE,
                'churn_mode': CONFIG.BT_CHURN_MODE,
                'protocol_only': CONFIG.BT_PROTOCOL_ONLY,
                'synthetic_chunk_size': CONFIG.BT_SYNTHETIC_CHUNK_SIZE,
                'synthetic_num_chunks': CONFIG.BT_SYNTHETIC_NUM_CHUNKS
            },
        ...
        if '@transformers' in CONFIG.MODEL_TYPE:
            ...
            # Custom text field name (default: 'sentence' for GLUE, use 'text' for AG News, IMDB, etc.)
            if CONFIG.DATA_TEXT_FIELD and CONFIG.DATA_TEXT_FIELD != "sentence":
                data_args['text_field'] = CONFIG.DATA_TEXT_FIELD
            # Split train for validation (for datasets without validation set, e.g., AG News)
            if CONFIG.DATA_SPLIT_TRAIN_FOR_VAL > 0:
                data_args['split_train_for_val'] = CONFIG.DATA_SPLIT_TRAIN_FOR_VAL
            ...
            if CONFIG.USE_LORA:
                ...
                config['model']['lora_r'] = CONFIG.LORA_R
                config['model']['lora_alpha'] = CONFIG.LORA_ALPHA
                config['model']['lora_dropout'] = CONFIG.LORA_DROPOUT
                if CONFIG.LORA_TARGET_MODULES:
                    config['model']['lora_target_modules'] = CONFIG.LORA_TARGET_MODULES
                self.logger.info(f"   🎯 LoRA enabled: r={CONFIG.LORA_R}, alpha={CONFIG.LORA_ALPHA}, dropout={CONFIG.LORA_DROPOUT}")
                if CONFIG.LORA_TARGET_MODULES:
                    self.logger.info(f"   🎯 LoRA target modules: {CONFIG.LORA_TARGET_MODULES}")
                else:
                    self.logger.info(f"   🎯 LoRA target modules: auto-detect based on model")
...
def generate_experiment_slurm_script(
    ...
    for key, value in exp_config.items():
        if isinstance(value, str):
            # Check if string is a numeric value (e.g., "1e-8" scientific notation)
            try:
                float_val = float(value)
                config_overrides.append(f'CONFIG.{key} = {float_val}')
            except ValueError:
