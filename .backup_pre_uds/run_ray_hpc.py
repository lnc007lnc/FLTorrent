...
class RayV2FederatedLearning:
    ...
    def start_federated_learning(self):
        ...
        for i, device_profile in enumerate(device_assignments):
            ...
            self.logger.info(f"🔧 Client {client_id} resources: num_cpus={client_resources.get('num_cpus', 0):.4f}, "
                           f"num_gpus={client_resources.get('num_gpus', 0):.4f}, "
                           f"memory={client_resources.get('memory', 0) / (1024**3):.2f}GB")
