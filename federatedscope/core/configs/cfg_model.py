...
def extend_model_cfg(cfg):
    ...
    # ---------------------------------------------------------------------- #
    # LoRA (Low-Rank Adaptation) options for transformer models
    # ---------------------------------------------------------------------- #
    cfg.model.use_lora = False  # Whether to use LoRA for parameter-efficient fine-tuning
    cfg.model.lora_r = 8  # LoRA rank (low rank dimension)
    cfg.model.lora_alpha = 16  # LoRA alpha scaling factor
    cfg.model.lora_dropout = 0.1  # LoRA dropout rate
    cfg.model.lora_target_modules = []  # Target modules for LoRA, empty means auto-detect
