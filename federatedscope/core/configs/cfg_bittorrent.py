...
def extend_bittorrent_cfg(cfg):
    ...
    cfg.bittorrent.comp_cmax = 2.5             # Maximum compensation factor (sensitivity knob, replaces hardcoded round-based c_max)

    # Overhead instrumentation
    cfg.bittorrent.collect_overhead_stats = False  # Per-round metadata bytes / scoring CPU / compensation CPU JSONL dump
    cfg.bittorrent.overhead_stats_dir = ''         # Output dir; empty -> derived from outdir/bittorrent_logs

    # Swarming-mechanism isolation knobs (P0-3)
    cfg.bittorrent.owner_only_serving = False  # Condition A: peer can only serve chunks it originally owned (no re-serving received chunks within the same round)
    cfg.bittorrent.collect_isolation_stats = False  # Per-round JSONL dump of isolation-experiment metrics (utility-rho, served_own/received, distinct sources, dup requests)
    ...
    cfg.bittorrent.churn_mode = 'mid_round_drop'  # Modes:
                                          #   'mid_round_drop' (recommended, matches paper §4.2):
                                          #       client trains normally; BT layer disconnects at random
                                          #       t' ∈ [0, timeout]; already-received chunks kept; server
                                          #       does not hang because client still sends model_para.
                                          #   'silent' / 'skip' / 'delayed': legacy modes; may deadlock
                                          #       server when combined with sample_client_num<client_num.
