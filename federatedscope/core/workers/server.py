...
class Server(BaseServer, ConnectionHandlerMixin):
    ...
    def __init__(self,
        ...
        self.client_sample_sizes = dict()  # 存储每个 client 的真实 sample_size
    ...
    def _perform_federated_aggregation(self):
        """
        Perform server-side aggregation for CFL, and completion tracking for
        BitTorrent P2P mode where clients aggregate locally.
        """
        ...
        if bt_enabled:
            logger.info(
                "[BT-FL] Server: Processing training completion signals - "
                "skipping server-side aggregation")
            ...
            for client_id, sync_info in train_msg_buffer.items():
                if isinstance(sync_info, dict) and sync_info.get(
                        'training_completed', False):
                    completed_clients.append(client_id)
                    total_samples += sync_info.get('sample_size', 0)
                    logger.debug(
                        f"[BT-FL] Server: Client {client_id} completed "
                        f"training with {sync_info.get('sample_size', 0)} "
                        "samples")
            ...
            logger.info(
                f"[BT-FL] Server: Round {self.state} completion tracking - "
                f"{aggregated_num} clients completed training with "
                f"{total_samples} total samples")

            return aggregated_num

        # CFL: standard server-side aggregation
        model_list = list(train_msg_buffer.values())
        if not model_list:
            logger.warning(
                f"Server: No client updates to aggregate at round "
                f"{self.state}")
            return 0

        agg_num = len(model_list)
        for i in range(self.model_num):
            if self.model_num > 1:
                client_feedback = [
                    (sample_size, model_para[i])
                    for sample_size, model_para in model_list
                ]
            else:
                client_feedback = model_list

            agg_info = {"client_feedback": client_feedback}
            if self._cfg.federate.use_ss:
                agg_info["recover_fun"] = self.recover_fun

            aggregated_model = self.aggregators[i].aggregate(agg_info)
            if self._cfg.federate.share_local_model:
                aggregated_model = merge_param_dict(
                    self.models[i].state_dict(), aggregated_model)
            self.aggregators[i].update(aggregated_model)

        logger.info(
            f"Server: Aggregated {agg_num} client updates for round "
            f"{self.state}")
        return agg_num
    ...
    def broadcast_model_para(self,
        ...
        # Track this round's active client subset for BT-wait quorum + start_bittorrent recipient list.
        # Only relevant when sample_client_num < client_num (per-round dropout); otherwise
        # this set equals all clients and the BT path behaves identically to before.
        if msg_type == 'model_para':
            self._this_round_active_clients = set(int(r) for r in receiver)
        ...
        if msg_type == 'model_para':
            if bt_enabled:
                message_content = {
                    ...
                    'aggregation_method': getattr(self._cfg.federate,
                                                  'method', 'FedAvg'),
                    'sample_client_num': len(receiver)
                    if 'receiver' in locals() else self.client_num,
                    'skip_weights': True,
                    'client_sample_sizes': self.client_sample_sizes  # 广播所有 client 的真实 sample_size
                }
            else:
                if self.model_num > 1:
                    message_content = [
                    ...
                    message_content = self.models[0].state_dict()
        else:
            message_content = {}
    ...
    def callback_funcs_model_para(self, message: Message):
        ...
        if bt_enabled and isinstance(content, dict) and content.get(
                'skip_weights', False):
            ...
            # CFL: handle traditional model parameter messages
            logger.info(
                f"Server: Received model parameters from client {sender} "
                f"for round {round}")
        ...
        # Store client update (sync info for BT, model parameters for CFL)
        if round == self.state:
        ...
        # Skip online aggregation for BitTorrent mode
        if self._cfg.federate.online_aggr and bt_enabled:
            logger.warning(
                "[BT-FL] Server: Online aggregation disabled for BitTorrent "
                "mode")
    ...
    def callback_funcs_for_join_in(self, message: Message):
        ...
        if 'info' in message.msg_type:
            ...
            # 兼容新旧格式：新格式包含 address 和 sample_size，旧格式只有 address
            if isinstance(content, dict) and 'address' in content:
            ...
            logger.info(f"🔗 [JOIN] Client joining with sender={sender}, address={address}, sample_size={sample_size}")
            ...
            # 存储 client 的 sample_size（用于 Cadence 加权聚合）
            if sample_size > 0:
                self.client_sample_sizes[sender] = sample_size
                logger.info(f"[BT-FL] Server: Stored sample_size={sample_size} for client {sender}")
    ...
    def trigger_bittorrent(self):
        ...
        # Per-round active client set (set by broadcast_model_para). Defaults to ALL clients
        # for backward compatibility (no dropout / sample_client_num == client_num).
        active = getattr(self, '_this_round_active_clients', None)
        if not active:
            active = set(range(1, self.client_num + 1))
        self._bt_active_clients = active
        n_active = len(active)

        # Get configuration
        chunks_per_client = getattr(getattr(self._cfg, 'chunk', None), 'num_chunks', 10)
        self.bt_expected_chunks = n_active * chunks_per_client

        # Broadcast start BitTorrent message to active clients only
        self.comm_manager.send(
            Message(msg_type='start_bittorrent',
                    sender=self.ID,
                    receiver=sorted(active),
                    state=self.state,
                    content={
                        'round': self.state,
    ...
    def _wait_for_all_bittorrent_completion(self):
        ...
        Uses _bt_active_clients (set in trigger_bittorrent) to support per-round dropout via
        cfg.federate.sample_client_num < client_num. Backward compatible (defaults to all clients).
        """
        start_time = time.time()
        check_interval = 2.0  # Check every 2 seconds
        last_completed_clients = -1  # Track number of clients completed last time

        active = getattr(self, '_bt_active_clients', set(range(1, self.client_num + 1)))
        n_expected = len(active)

        logger.info(f"[BT] Starting synchronous wait for {n_expected} active clients to complete BitTorrent exchange")

        while True:
            # Count only completions from this round's active set (defensive: ignore late
            # reports from clients that weren't sampled this round).
            completed_clients = sum(1 for cid in self.bittorrent_completion_status if cid in active)
            if completed_clients >= n_expected:
                total_time = time.time() - start_time
                logger.info(f"[BT] ✅ All {n_expected} active clients completed BitTorrent exchange in {total_time:.2f}s")
            ...
            if elapsed_time > self.bt_timeout:
                ...
                logger.warning(f"[BT] Only {completed_clients}/{n_expected} active clients completed")
            ...
            if completed_clients != last_completed_clients:
                logger.info(f"[BT] 🕰️ BitTorrent progress: {completed_clients}/{n_expected} clients completed, {elapsed_time:.1f}s elapsed")
    ...
    def callback_funcs_for_bittorrent_complete(self, message):
        ...
        if hasattr(self, 'bittorrent_completion_status'):
            ...
            active = getattr(self, '_bt_active_clients', set(range(1, self.client_num + 1)))
            completed_count = sum(1 for cid in self.bittorrent_completion_status if cid in active)
            logger.info(f"[BT] Progress: {completed_count}/{len(active)} active clients completed BitTorrent exchange")
    ...
    def check_bittorrent_state(self):
        ...
        # Check if all active clients completed (per-round dropout aware)
        active = getattr(self, '_bt_active_clients', set(range(1, self.client_num + 1)))
        n_active = len(active)
        completed_active = sum(1 for cid in self.bittorrent_completion_status if cid in active)
        if completed_active >= n_active:
            self._handle_bittorrent_completion()
        elif completed_active >= n_active * 0.8:
            ...
            if remaining_time < 5.0:  # Only 5 seconds left
                logger.info("[BT] 80% active clients completed, proceeding...")
                self._handle_bittorrent_completion()
    ...
    def _handle_bittorrent_completion(self):
        ...
        # Calculate success rate (per-round dropout aware)
        active = getattr(self, '_bt_active_clients', set(range(1, self.client_num + 1)))
        n_active = len(active)
        success_count = sum(1 for cid, status in self.bittorrent_completion_status.items()
                            if cid in active and status >= self.bt_expected_chunks)

        if success_count == n_active:
            logger.info(f"[BT] All {n_active} active clients successfully collected all chunks")
        else:
            logger.warning(f"[BT] {success_count}/{n_active} active clients collected all chunks")
