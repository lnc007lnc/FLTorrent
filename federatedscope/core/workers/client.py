...
from federatedscope.core.overhead_stats import OverheadStatsCollector
from federatedscope.core.compression.sparsification import (
    topk_sparsify_state_dict,
    state_dict_diff,
    state_dict_add,
    state_dict_zeros_like,
)
...
class Client(BaseClient):
    ...
    def __init__(self,
        ...
        self._overhead_collector = self._init_overhead_collector()

    def _init_overhead_collector(self):
        """Build per-client overhead stats collector if either overhead OR isolation stats are enabled."""
        bt_cfg = getattr(self._cfg, 'bittorrent', None)
        if bt_cfg is None:
            return None
        want_overhead  = getattr(bt_cfg, 'collect_overhead_stats',  False)
        want_isolation = getattr(bt_cfg, 'collect_isolation_stats', False)
        if not (want_overhead or want_isolation):
            return None
        out_dir = getattr(bt_cfg, 'overhead_stats_dir', '') or ''
        if not out_dir:
            base = getattr(self._cfg, 'outdir', '') or '.'
            out_dir = f"{base}/bittorrent_logs/overhead"
        out_file = f"{out_dir}/client_{self.ID}.jsonl"
        try:
            return OverheadStatsCollector(client_id=self.ID, output_file=out_file)
        except Exception as e:
            logger.warning(f"[OverheadStats] Client {self.ID}: failed to init collector at {out_file}: {e}")
            return None

    def _attach_overhead_collector(self, manager):
        """Inject overhead collector into a chunk/bt manager (no-op if disabled)."""
        coll = getattr(self, '_overhead_collector', None)
        if coll is not None and manager is not None:
            manager._overhead_collector = coll

    # ===== Sparse-delta DFL baselines: CHOCO-SGD + GossipFL =====
    def _sparse_baseline_mode(self):
        """Return 'choco_sgd' / 'gossipfl' / None depending on which baseline is active."""
        if hasattr(self._cfg, 'choco_sgd') and self._cfg.choco_sgd.enable:
            return 'choco_sgd'
        if hasattr(self._cfg, 'gossipfl') and self._cfg.gossipfl.enable:
            return 'gossipfl'
        if hasattr(self._cfg, 'topology') and hasattr(self._cfg.topology, 'gossip') \
                and self._cfg.topology.gossip.enable:
            mode = self._cfg.topology.gossip.mode
            if mode in ('choco_sgd', 'gossipfl'):
                return mode
        return None

    def _choco_sgd_enabled(self):  # backwards-compatible alias used by chunk-save hook
        return self._sparse_baseline_mode() is not None

    def _sparse_baseline_params(self):
        """Return (k_ratio, block_size, label) tuple for the active baseline."""
        m = self._sparse_baseline_mode()
        if m == 'choco_sgd':
            return float(self._cfg.choco_sgd.k_ratio), 0, 'CHOCO-SGD'
        if m == 'gossipfl':
            return float(self._cfg.gossipfl.k_ratio), int(self._cfg.gossipfl.block_size), 'GossipFL'
        return 1.0, 0, 'NONE'

    def _init_choco_state(self, reference_state_dict):
        """Lazily initialize sparse-baseline trackers (zeros, same shape as model)."""
        if not hasattr(self, '_choco_x_hat_self') or self._choco_x_hat_self is None:
            self._choco_x_hat_self = state_dict_zeros_like(reference_state_dict)
            self._choco_x_hat_neighbors = {}
            self._choco_last_bytes_on_wire = 0
            k_ratio, block_size, label = self._sparse_baseline_params()
            logger.info(f"[{label}] Client {self.ID}: state initialized "
                        f"(k_ratio={k_ratio}, block_size={block_size})")

    def _choco_make_sparse_payload(self, current_model):
        """Compute sparse delta q_i = TopK(x_i - x̂_self), update self tracker, return sparse_state_dict.
        Shared by CHOCO-SGD and GossipFL (only differs in k_ratio and optional block_size)."""
        k_ratio, block_size, label = self._sparse_baseline_params()
        x_i_state = {k: v.detach().clone().cpu() for k, v in current_model.state_dict().items()}
        self._init_choco_state(x_i_state)
        delta_raw = state_dict_diff(x_i_state, self._choco_x_hat_self)
        # Drop None values (non-float keys)
        delta_for_sparse = {k: v for k, v in delta_raw.items() if v is not None}
        sparse_delta, info = topk_sparsify_state_dict(
            delta_for_sparse, k_ratio=k_ratio, skip_bn=True, block_size=block_size)
        # Reattach non-float fields (BN num_batches_tracked etc.) as-is
        for k, v in x_i_state.items():
            if k not in sparse_delta:
                sparse_delta[k] = v
        # Update self tracker x̂_self ← x̂_self + q_i
        state_dict_add(self._choco_x_hat_self, sparse_delta, alpha=1.0)
        self._choco_last_bytes_on_wire = info['bytes_on_wire']
        logger.info(f"[{label}] Client {self.ID}: round {self.state} sent "
                    f"{info['bytes_on_wire']} bytes-on-wire ({info['compression_ratio']*100:.2f}% kept)")
        return sparse_delta
    ...
    def join_in(self):
        ...
        """
        # 获取真实的训练数据样本数（用于 Cadence 加权聚合）
        sample_size = 0
        if hasattr(self, 'data') and self.data is not None:
            # ClientData 类使用 train_data 属性存储原始训练数据集
            if hasattr(self.data, 'train_data') and self.data.train_data is not None:
                sample_size = len(self.data.train_data)
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if 'ss' in message.msg_type:
            ...
            if len(self.msg_buffer['train']
                ...
                bt_enabled = hasattr(self._cfg, 'bittorrent') and \
                    self._cfg.bittorrent.enable
                if bt_enabled:
                    ...
                    logger.info(
                        f"[BT-FL] Client {self.ID}: Sending training "
                        f"completion signal for round {self.state} with "
                    ...
                    model_para_to_send = first_aggregate_model_para[0] \
                    ...
                    self.comm_manager.send(
                        Message(msg_type='model_para',
                                ...
                                state=self.state,
                                timestamp=timestamp,
                                content=(sample_size, model_para_to_send)))
            ...
            if isinstance(content, dict) and content.get('skip_weights', False):
                # 存储 server 广播的 client_sample_sizes（用于加权聚合）
                if 'client_sample_sizes' in content:
                    self._client_sample_sizes = content['client_sample_sizes']
                    logger.info(f"[BT-FL] Client {self.ID}: Received sample sizes for {len(self._client_sample_sizes)} clients")
    ...
    def callback_funcs_for_assign_id(self, message: Message):
        ...
        if hasattr(self, 'comm_manager') and hasattr(self.comm_manager, 'server_funcs'):
            ...
            if not hasattr(self, 'chunk_manager'):
                ...
                self._attach_overhead_collector(self.chunk_manager)
    ...
    def _save_local_model_after_training(self):
        ...
        try:
            ...
            # CHOCO-SGD baseline: replace state_dict with sparsified delta before chunking,
            # restore right after. Self tracker x̂_self is updated inside _choco_make_sparse_payload.
            _orig_state = None
            if self._choco_sgd_enabled():
                sparse_delta = self._choco_make_sparse_payload(model)
                _orig_state = {k: v.detach().clone() for k, v in model.state_dict().items()}
                model.load_state_dict(sparse_delta, strict=False)
            ...
            # Restore the local model after chunking (CHOCO-SGD only)
            if _orig_state is not None:
                model.load_state_dict(_orig_state, strict=False)
    ...
    def _get_expected_client_sample_sizes(self, round_num):
        ...
        try:
            # Method 0: 优先使用 server 广播的 sample_sizes（最可靠的来源）
            if hasattr(self, '_client_sample_sizes') and self._client_sample_sizes:
                logger.debug(f"[BT-FL] Client {self.ID}: Using server-broadcast sample sizes for {len(self._client_sample_sizes)} clients")
                return self._client_sample_sizes
    ...
    def _apply_post_aggregation_compensation(self, aggregated_model, completed_client_models, baseline_params):
        ...
        # comp_cmax controls the steady-state c_max (round>=9). The original
        # round<9 boost (c_max=3 for warmup) is preserved unchanged so that the
        # default config value (2.5) reproduces baseline behavior exactly.
        try:
            cmax_steady = float(self._cfg.bittorrent.comp_cmax)
        except Exception:
            cmax_steady = 2.5
        ...
        coll = getattr(self, '_overhead_collector', None)
        if coll is not None:
            coll.record_compensation((time.perf_counter() - _comp_t0) * 1000.0)
            coll.flush_round(self.state)
    ...
    def _aggregate_model_from_chunks(self, round_num, aggregator_type='fedavg'):
        ...
        try:
            ...
            # Gossip mode: filter to only aggregate neighbor models
            if self._is_gossip_mode():
                neighbors = self._get_topology_neighbors()
                neighbors_with_self = set(neighbors) | {self.ID}
                original_count = len(available_clients)
                available_clients = [c for c in available_clients if c in neighbors_with_self]
                logger.info(f"[BT-FL] Client {self.ID}: Gossip mode - filtered from {original_count} to {len(available_clients)} neighbors: {available_clients}")
            ...
            # NOTE: Skip baseline injection in gossip mode to keep strict neighbor-only semantics.
            if self._is_gossip_mode():
                logger.info(f"[BT-FL] Client {self.ID}: Gossip mode - skipping baseline injection")
            elif expected_sample_sizes:
            ...
            # Perform aggregation: gossip mode uses custom aggregation, otherwise use standard aggregator
            if self._is_gossip_mode():
                # Gossip-style aggregation: weighted average with neighbors
                # Support multiple gossip iterations for better convergence
                iterations = self._cfg.topology.gossip.iterations
                aggregated_model = None
                for i in range(iterations):
                    aggregated_model = self._gossip_aggregate(completed_client_models, baseline_params)
                    # Update completed_client_models with new self_model for next iteration
                    if iterations > 1 and i < iterations - 1:
                        for j, (ss, mp, cid, cov) in enumerate(completed_client_models):
                            if cid == self.ID:
                                completed_client_models[j] = (ss, aggregated_model, cid, cov)
                                break
                logger.info(f"[BT-FL] Client {self.ID}: Completed {iterations} gossip iteration(s) with {len(completed_client_models)} neighbor models")
            else:
                # Standard aggregation via built-in aggregator
                agg_info = {
            ...
            if enable_compensation and aggregated_model is not None and isinstance(aggregated_model, dict):
                ...
                # Even when compensation is disabled, the per-round overhead/isolation collector
                # still needs to flush its accumulated metrics (HAVE/bitfield bytes, served_own/
                # received counts, utility-rho, etc). Compensation is normally where we flush;
                # without it, we must flush here.
                _coll = getattr(self, '_overhead_collector', None)
                if _coll is not None:
                    _coll.flush_round(self.state)
            ...
            # agg_info also holds references to these tensors (only defined in non-gossip mode)
            del completed_client_models
            if 'agg_info' in locals():
                del agg_info
    ...
    def _is_gossip_mode(self):
        """Check if gossip mode is enabled"""
        return (hasattr(self._cfg, 'topology') and
                hasattr(self._cfg.topology, 'gossip') and
                self._cfg.topology.gossip.enable)

    def _get_topology_neighbors(self):
        """Get topology neighbors for current node"""
        if hasattr(self, 'topology_neighbors') and self.topology_neighbors:
            return list(self.topology_neighbors)
        return []

    def _gossip_aggregate(self, client_models, baseline_params):
        """
        Gossip-style aggregation: weighted average with neighbors

        Args:
            client_models: List of (sample_size, model_params, client_id, coverage)
            baseline_params: Baseline model parameters for missing values

        Returns:
            dict: Aggregated model parameters
        """
        mode = self._cfg.topology.gossip.mode
        mixing_weight = self._cfg.topology.gossip.mixing_weight

        # Separate self model and neighbor models
        self_model = None
        neighbor_models = []
        for sample_size, model_params, client_id, coverage in client_models:
            if client_id == self.ID:
                self_model = model_params
            else:
                neighbor_models.append((model_params, client_id))

        # Use baseline if self model not available
        if self_model is None:
            self_model = baseline_params
            logger.debug(f"[BT-FL] Client {self.ID}: Using baseline as self model for gossip aggregation")

        # If no neighbors available, return self model
        if not neighbor_models:
            logger.warning(f"[BT-FL] Client {self.ID}: No neighbor models available for gossip aggregation")
            return self_model

        if mode in ('choco_sgd', 'gossipfl'):
            # Both CHOCO-SGD and GossipFL share: neighbors send sparse deltas q_j; we maintain
            # per-neighbor trackers x̂_j. They differ only in the consensus step:
            #   CHOCO-SGD:  x_i ← x_i + γ · Σ_j (x̂_j - x̂_self)        (Koloskova ICML 2019)
            #   GossipFL:   x_i ← (1/(N+1)) (x_i + Σ_j x̂_j)            (Tang ICML 2022, simplified
            #                                                            without BAR scheduling)
            # IMPORTANT: self_model from completed_client_models is the SPARSE DELTA we sent,
            # not x_i. We must read the actual local trained model from the trainer.
            local_model = None
            if hasattr(self.trainer, 'ctx') and hasattr(self.trainer.ctx, 'model'):
                local_model = self.trainer.ctx.model
            elif hasattr(self.trainer, 'model'):
                local_model = self.trainer.model
            if local_model is None:
                logger.error(f"[{mode.upper()}] Client {self.ID}: cannot access local trained model; falling back to neighbor_avg")
                mode = 'neighbor_avg'
            else:
                x_i = {k: v.detach().clone() for k, v in local_model.state_dict().items()}
                self._init_choco_state(x_i)
                # Update per-neighbor trackers: x̂_j ← x̂_j + q_j
                for q_j, j_id in neighbor_models:
                    if j_id not in self._choco_x_hat_neighbors:
                        self._choco_x_hat_neighbors[j_id] = state_dict_zeros_like(q_j)
                    state_dict_add(self._choco_x_hat_neighbors[j_id], q_j, alpha=1.0)

                # Both algorithms use the same γ-mixing form: x_i ← x_i + γ Σ_j (x̂_j - x̂_self).
                # The differentiation is in the sparsification (global vs block-wise top-k),
                # which has already been applied in _choco_make_sparse_payload before the
                # delta was sent. GossipFL paper's uniform-average mixing rule was found to
                # diverge under zero-initialised trackers (verified experimentally on 25-client
                # mesh deg=4 — model stayed at ~50% random for 10 rounds). γ-mixing keeps
                # convergence stable while preserving the sparsified-gossip family semantic.
                if mode == 'choco_sgd':
                    gamma = float(self._cfg.choco_sgd.gamma)
                    label = 'CHOCO-SGD'
                else:  # gossipfl
                    gamma = float(self._cfg.gossipfl.gamma)
                    label = 'GossipFL'
                mixed = {k: v.clone() if hasattr(v, 'clone') else v for k, v in x_i.items()}
                for j_id, x_hat_j in self._choco_x_hat_neighbors.items():
                    diff = state_dict_diff(x_hat_j, self._choco_x_hat_self)
                    state_dict_add(mixed, diff, alpha=gamma)
                logger.info(f"[{label}] Client {self.ID}: round {self.state} mixed with "
                            f"{len(self._choco_x_hat_neighbors)} neighbor trackers (γ={gamma})")
                return mixed

        if mode == 'pairwise':
            # Randomly select one neighbor for averaging
            import random
            neighbor_model, neighbor_id = random.choice(neighbor_models)
            logger.debug(f"[BT-FL] Client {self.ID}: Pairwise gossip with neighbor {neighbor_id}")
            return self._weighted_avg_two_models(self_model, neighbor_model, mixing_weight)

        elif mode == 'neighbor_avg':
            # Average with all neighbors using equal weights
            # Fix: handle case where neighbor models are missing some keys
            aggregated = {}
            for key in self_model:
                # Count neighbors that have this key
                neighbors_with_key = sum(1 for nm, _ in neighbor_models if key in nm)
                if neighbors_with_key > 0:
                    # Recalculate neighbor weight based on actual neighbors with this key
                    actual_neighbor_weight = (1 - mixing_weight) / neighbors_with_key
                    aggregated[key] = self_model[key] * mixing_weight
                    for neighbor_model, neighbor_id in neighbor_models:
                        if key in neighbor_model:
                            aggregated[key] = aggregated[key] + neighbor_model[key] * actual_neighbor_weight
                else:
                    # No neighbor has this key, keep self model's value
                    aggregated[key] = self_model[key]
            logger.debug(f"[BT-FL] Client {self.ID}: Neighbor avg gossip with {len(neighbor_models)} neighbors, "
                        f"self_weight={mixing_weight:.2f}")
            return aggregated

        elif mode == 'push_sum':
            # Push-sum style: equal weight for self and all neighbors
            # Fix: handle case where neighbor models are missing some keys
            aggregated = {}
            for key in self_model:
                # Count neighbors that have this key
                neighbors_with_key = sum(1 for nm, _ in neighbor_models if key in nm)
                # Total participants = 1 (self) + neighbors with this key
                total_participants = 1 + neighbors_with_key
                weight_per_node = 1.0 / total_participants
                aggregated[key] = self_model[key] * weight_per_node
                for neighbor_model, neighbor_id in neighbor_models:
                    if key in neighbor_model:
                        aggregated[key] = aggregated[key] + neighbor_model[key] * weight_per_node
            logger.debug(f"[BT-FL] Client {self.ID}: Push-sum gossip with {len(neighbor_models)} neighbors")
            return aggregated

        else:
            # Default to neighbor_avg if unknown mode
            logger.warning(f"[BT-FL] Client {self.ID}: Unknown gossip mode '{mode}', falling back to neighbor_avg")
            aggregated = {}
            for key in self_model:
                neighbors_with_key = sum(1 for nm, _ in neighbor_models if key in nm)
                if neighbors_with_key > 0:
    ...
    def _weighted_avg_two_models(self, model1, model2, weight1):
        """
        Weighted average of two models

        Args:
            model1: First model parameters dict
            model2: Second model parameters dict
            weight1: Weight for model1 (model2 weight = 1 - weight1)

        Returns:
            dict: Weighted average model parameters
        """
        weight2 = 1.0 - weight1
        result = {}
        # Iterate over union of all keys from both models
        all_keys = set(model1.keys()) | set(model2.keys())
        for key in all_keys:
            if key in model1 and key in model2:
                result[key] = model1[key] * weight1 + model2[key] * weight2
            elif key in model1:
                # model2 missing this key: keep original value (not scaled)
                result[key] = model1[key]
            else:
                # model1 missing this key: keep model2's value (not scaled)
                result[key] = model2[key]
        return result
    ...
    def callback_funcs_for_start_bittorrent(self, message):
        ...
        # 🔧 GOSSIP FIX: Override server's expected_chunks in Gossip mode
        # Server sends total chunks (all clients), but Gossip mode only needs neighbor chunks
        if self._is_gossip_mode() and hasattr(self, 'topology_neighbors') and self.topology_neighbors:
            chunks_per_client = self._cfg.chunk.num_chunks if hasattr(self._cfg, 'chunk') else 8
            expected_chunks = (len(self.topology_neighbors) + 1) * chunks_per_client
            logger.info(f"[BT] Client {self.ID}: Gossip mode - overriding expected_chunks from server to {expected_chunks} (self + {len(self.topology_neighbors)} neighbors)")
    ...
    def _start_bittorrent_exchange(self, neighbors, round_num):
        ...
        self._attach_overhead_collector(self.bt_manager)
        ...
        # 🔧 GOSSIP FIX: In Gossip mode, only expect chunks from neighbors + self
        # This prevents incorrect expected_total calculation that would cause timeout issues
        if self._is_gossip_mode() and hasattr(self, 'topology_neighbors') and self.topology_neighbors:
            # Gossip mode: only expect chunks from self + neighbors
            total_clients = len(self.topology_neighbors) + 1  # neighbors + self
            logger.info(f"[BT] Client {self.ID}: Gossip mode - expecting chunks from {total_clients} clients (self + {len(self.topology_neighbors)} neighbors: {self.topology_neighbors})")
        else:
    ...
    def _run_bittorrent_exchange_loop(self, expected_chunks):
        ...
        try:
            ...
            # Simulate client failure during BitTorrent exchange.
            #
            # Modes:
            #   'mid_round_drop' (RECOMMENDED, matches spec §4.2):
            #     Client trains normally, starts BT exchange normally, then at a
            #     uniformly-random time t' ∈ [0, t] disconnects from the BT layer:
            #     stops responding to REQUEST/PIECE/HAVE, stops issuing new
            #     REQUESTs. Already-received chunks are kept (chunk_write_queue
            #     drains). At round end, aggregation runs on whatever was
            #     received; client still sends model_para to server (so server
            #     does not hang on min_received_num quorum).
            #     This avoids the FEDERATE_SAMPLE_CLIENT_NUM staleness issue
            #     where sampled-out clients' models drift behind the active set.
            #
            #   'silent' / 'skip' (LEGACY): exit BT loop early. May deadlock
            #     server's model_para wait if used with sample_client_num.
            if (hasattr(self._cfg, 'bittorrent') and
                ...
                if churn_mode == 'mid_round_drop':
                    # Schedule a drop time uniformly in [0, t]. The BT manager
                    # silently drops all REQUEST/PIECE/HAVE traffic past this
                    # deadline. Local training, chunk save, and final aggregation
                    # all proceed normally.
                    drop_offset = _churn_random.uniform(0.0, timeout_duration)
                    drop_at = time.time() + drop_offset
                    if hasattr(self, 'bt_manager') and self.bt_manager is not None:
                        self.bt_manager._drop_at = drop_at
                        logger.info(f"[Churn] Client {self.ID}: mid_round_drop scheduled at t+{drop_offset:.1f}s (round {self.state})")
                    else:
                        logger.warning(f"[Churn] Client {self.ID}: bt_manager not initialized, cannot schedule drop")
                    # IMPORTANT: do NOT return here — continue with normal BT loop

                elif churn_mode == 'silent':
                    # LEGACY: Silent failure - sleeps for 80% of timeout, then
                    # reports 0-chunks. Causes server to hang if used with
                    # sample_client_num < client_num. Kept for backward compat.
                    ...
                    # LEGACY: immediately exit BT loop. May deadlock server.
