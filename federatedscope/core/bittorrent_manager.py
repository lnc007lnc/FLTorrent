...
class BitTorrentManager:
    ...
    def __init__(self, client_id: int, round_num: int, chunk_manager, comm_manager, neighbors: List[int], cfg=None, streaming_manager=None):
        ...
        # Mid-round dropout (E4 churn): set by client at round start; if not None
        # and time.time() >= _drop_at, this peer silently drops all BT traffic.
        # Only an atomic value compare — no lock needed; safe in callback threads.
        self._drop_at = None
    ...
    def _is_currently_dropped(self) -> bool:
        """Mid-round dropout (E4 churn) check. Returns True iff a drop has been
        scheduled and the deadline has passed. Pure atomic value compare; safe to
        call from any thread (callback or worker)."""
        drop_at = self._drop_at
        return drop_at is not None and time.time() >= drop_at
    ...
    def handle_request(self, sender_id: int, round_num: int, source_client_id: int, chunk_id: int):
        ...
        # E4 churn: silently drop (spec §4.2 "不响应 REQUEST")
        if self._is_currently_dropped():
            return
    ...
    def handle_piece(self, sender_id: int, round_num: int, source_client_id: int, chunk_id: int, chunk_data, checksum: str):
        ...
        # E4 churn: drop new incoming pieces after disconnect (in-flight already-
        # written chunks are kept by chunk_write_queue; spec §4.2)
        if self._is_currently_dropped():
            return False
        ...
        # P0-3 isolation metric: count received chunks + utility numerator + sources
        _coll = getattr(self, '_overhead_collector', None)
        if _coll is not None:
            _imp = self._get_chunk_importance_score(chunk_key) or 0.0
            _coll.record_received_chunk(source_id=source_client_id, importance=_imp)
    ...
    def handle_have(self, sender_id: int, round_num: int, source_client_id: int, chunk_id: int, importance_score: float = 0.0):
        ...
        # E4 churn: dropped peers don't process incoming HAVEs (consistent with
        # being network-disconnected; we won't issue further REQUESTs anyway)
        if self._is_currently_dropped():
            return
    ...
    def _fill_pending_queue(self):
        ...
        if needed_chunks:
            # P0-3 isolation metric: record total importance of needed chunks (utility-rho denominator)
            # Note: this is recomputed each time fill_pending_queue runs; the last call
            # within a round captures the final "total demand" denominator for that round.
            _coll = getattr(self, '_overhead_collector', None)
            if _coll is not None:
                _coll.record_total_importance(sum(x['importance'] for x in needed_chunks))
    ...
    def _send_bitfield(self, peer_id: int):
        ...
        if coll is not None:
            coll.record_bitfield(n_entries=len(bitfield_list), with_importance=True)
    ...
    def _broadcast_have(self, round_num: int, source_client_id: int, chunk_id: int):
        ...
        # E4 churn: dropped peers don't broadcast HAVE (spec §4.2 "不广播 HAVE")
        if self._is_currently_dropped():
            return
        ...
        coll = getattr(self, '_overhead_collector', None)
        if coll is not None and self.neighbors:
            coll.record_have(recipients=len(self.neighbors), with_importance=True)
    ...
    def _send_request(self, peer_id: int, source_id: int, chunk_id: int):
        ...
        # E4 churn: don't issue new REQUESTs after disconnect
        if self._is_currently_dropped():
            return False

        chunk_key = (self.round_num, source_id, chunk_id)

        # P0-3 isolation metric: count requests + dedup as duplicate
        _coll = getattr(self, '_overhead_collector', None)
        if _coll is not None:
            _is_dup = self.retry_count.get((source_id, chunk_id), 0) > 0
            _coll.record_request_sent(is_duplicate=_is_dup)
    ...
    def _upload_worker_loop(self):
        ...
        while not self.is_stopped:
            try:
                ...
                # E4 churn: drop queued upload after mid-round disconnect.
                # Saves a DB read; _send_piece would also block this anyway.
                if self._is_currently_dropped():
                    self._upload_task_queue.task_done()
                    continue

                # P0-3 Condition A: owner-only serving — drop the request if the
                # chunk is not in our own_chunks_set (i.e., it was received this
                # round, not originally owned).
                _coll = getattr(self, '_overhead_collector', None)
                _is_own = (round_num, source_id, chunk_id) in getattr(self, 'own_chunks_set', set())
                _owner_only = (self.cfg and hasattr(self.cfg, 'bittorrent')
                               and getattr(self.cfg.bittorrent, 'owner_only_serving', False))
                if _owner_only and not _is_own:
                    if _coll is not None:
                        _coll.record_serve(is_own=False, dropped=True)
                    # Throttled INFO log: 1st drop per round at INFO, rest at DEBUG.
                    if not hasattr(self, '_owner_only_drop_logged_round') or self._owner_only_drop_logged_round != round_num:
                        self._owner_only_drop_logged_round = round_num
                        logger.info(f"[UploadWorker] Client {self.client_id} round {round_num}: owner_only mode active; dropping serve of received chunk {source_id}:{chunk_id} to peer {sender_id} (further drops this round at debug level)")
                    else:
                        logger.debug(f"[UploadWorker] Client {self.client_id}: owner_only drop {source_id}:{chunk_id} -> peer {sender_id}")
                    self._upload_task_queue.task_done()
                    continue
                ...
                try:
                    ...
                    if chunk_data is not None:
                        ...
                        if success:
                            processed_count += 1
                            if _coll is not None:
                                _coll.record_serve(is_own=_is_own, dropped=False)
    ...
    def _send_piece(self, peer_id: int, round_num: int, source_client_id: int, chunk_id: int, chunk_data):
        ...
        # E4 churn: dropped peers don't send PIECEs (spec §4.2 "不发送 PIECE")
        if self._is_currently_dropped():
            return False
