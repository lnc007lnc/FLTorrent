...
def load_external_data(config=None):
    ...
    def load_huggingface_datasets_data(name, splits=None, config=None):
        ...
        if "load_disk_dir" in raw_args:
            ...
            # For datasets like AG News without a parent collection, use name as path
            # For GLUE datasets, path='glue', name='sst2'
            # For top-level datasets, path='ag_news', name=None
            if config.data.root:
                dataset = datasets.load_dataset(path=config.data.root,
                                                name=name,
                                                **filtered_args)
            else:
                dataset = datasets.load_dataset(path=name,
                                                **filtered_args)
        if config.model.type.endswith('transformers'):
        ...
        # Support custom text field name (default: 'sentence' for GLUE, 'text' for AG News)
        text_field = raw_args.get('text_field', 'sentence')
        label_field = raw_args.get('label_field', 'label')

        for split in dataset:
            x_all = [i[text_field] for i in dataset[split]]
            targets = [i[label_field] for i in dataset[split]]
        ...
        # Build data_split_dict, handling missing validation set
        data_split_dict = {
            'train': [(x, y)
                      for x, y in zip(dataset['train'][0], dataset['train'][1])
                      ],
            'val': [],  # Will be populated below
            'test': [
                (x, y) for x, y in zip(dataset['test'][0], dataset['test'][1])
            ] if (set(dataset['test'][1]) - set([-1])) else None,
        }

        # Handle validation set: check if 'validation' exists in dataset
        if 'validation' in dataset:
            data_split_dict['val'] = [(x, y) for x, y in zip(
                dataset['validation'][0], dataset['validation'][1])]
        elif "split_train_for_val" in raw_args and raw_args["split_train_for_val"] > 0:
            # If no validation set, split from train (e.g., for AG News)
            # split_train_for_val: ratio of train to use as validation (e.g., 0.1)
            val_ratio = raw_args["split_train_for_val"]
            train_data = data_split_dict['train']
            split_idx = int(len(train_data) * (1 - val_ratio))
            # Shuffle before splitting
            from random import shuffle as random_shuffle
            shuffled_indices = list(range(len(train_data)))
            random_shuffle(shuffled_indices)
            train_indices = shuffled_indices[:split_idx]
            val_indices = shuffled_indices[split_idx:]
            data_split_dict['train'] = [train_data[i] for i in train_indices]
            data_split_dict['val'] = [train_data[i] for i in val_indices]
            logger.info(f"Split train for validation: train={len(data_split_dict['train'])}, "
                       f"val={len(data_split_dict['val'])}")
        original_train_size = len(data_split_dict["train"])

        if "half_val_dummy_test" in raw_args and raw_args[
                "half_val_dummy_test"] and 'validation' in dataset:
