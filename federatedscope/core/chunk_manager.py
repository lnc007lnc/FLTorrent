...
class ChunkManager:
    ...
    def compute_chunk_importance(self, params: Dict[str, np.ndarray], chunks_info: List[Dict],
        ...
        if coll is not None:
            coll.record_scoring((time.perf_counter() - _score_t0) * 1000.0)
    ...
    # ==================== Experiment 3: Protocol Statistics ====================
    def get_protocol_statistics(self) -> Dict[str, Any]:
        """
        Get protocol-level statistics for scalability analysis.

        Returns:
            Dictionary containing:
            - control_plane_bytes: Bytes used for control messages (bitfield, have, request, etc.)
            - data_plane_bytes: Bytes used for actual chunk data
            - messages_per_round: Number of messages sent/received per round
            - completion_rate: Ratio of successful chunk exchanges
            - cache_stats: Cache hit/miss statistics
        """
        stats = {
            'control_plane_bytes': getattr(self, '_control_bytes_sent', 0),
            'data_plane_bytes': getattr(self, '_data_bytes_sent', 0),
            'messages_per_round': getattr(self, '_message_count', 0),
            'completion_rate': 0.0,
            'cache_stats': {},
        ...
        total_exchanges = getattr(self, '_total_exchanges', 0)
        completed_exchanges = getattr(self, '_completed_exchanges', 0)
        if total_exchanges > 0:
            stats['completion_rate'] = completed_exchanges / total_exchanges

        # Get cache statistics if available
        if hasattr(self, 'chunk_cache') and self.chunk_cache is not None:
            try:
                if hasattr(self.chunk_cache, 'get_stats'):
                    stats['cache_stats'] = self.chunk_cache.get_stats()
                if hasattr(self.chunk_cache, 'get_chunk_count'):
                    stats['total_chunks_stored'] = self.chunk_cache.get_chunk_count()
            except Exception as e:
                logger.debug(f"[ChunkManager] Could not get cache stats: {e}")

        return stats

    def increment_protocol_stats(self, control_bytes: int = 0, data_bytes: int = 0,
                                  message_count: int = 0, exchange_completed: bool = False):
        """
        Increment protocol statistics counters.

        Args:
            control_bytes: Number of control plane bytes to add
            data_bytes: Number of data plane bytes to add
            message_count: Number of messages to add
            exchange_completed: Whether an exchange was completed successfully
        """
        if not hasattr(self, '_control_bytes_sent'):
            self._control_bytes_sent = 0
        if not hasattr(self, '_data_bytes_sent'):
            self._data_bytes_sent = 0
        if not hasattr(self, '_message_count'):
            self._message_count = 0
        if not hasattr(self, '_total_exchanges'):
            self._total_exchanges = 0
        if not hasattr(self, '_completed_exchanges'):
            self._completed_exchanges = 0

        self._control_bytes_sent += control_bytes
        self._data_bytes_sent += data_bytes
        self._message_count += message_count
        self._total_exchanges += 1
        if exchange_completed:
            self._completed_exchanges += 1
